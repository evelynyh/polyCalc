...
public final class RatPoly {
            ...
            if (lst.get(0).getExpt() < newTerm.getExpt()) {
                ...
                for (int i = 0; i < s - 1; i++) {
                    if (lst.get(i).getExpt() == newTerm.getExpt()) {
                        if ((lst.get(i).add(newTerm)).isZero()) { //removes anything negated
                            lst.remove(i);
                           // s--;
                        } else {
                            lst.set(i, lst.get(i).add(newTerm));
                        }
                    } else if (lst.get(i).getExpt() > newTerm.getExpt() && newTerm.getExpt() > lst.get(i + 1).getExpt()) {
                        lst.add(i + 1, newTerm);
                        i++; //compensates for addition to lst
                    }
                }
                if (lst.get(lst.size() - 1).getExpt() == newTerm.getExpt()) {
                    if ((lst.get(lst.size() - 1).add(newTerm)).isZero()) { //removes anything negated
                        lst.remove(lst.size() - 1);
                    } else {
                        lst.set(lst.size() - 1, lst.get(lst.size()- 1).add(newTerm));
                    }
    ...
    public RatPoly negate() {
        ...
        if(isNaN()) {
            ret.terms.add( RatTerm.NaN);
            return ret;
        }
        for(RatTerm r : terms) {
            ret.terms.add(r.negate());
        }
        checkRep();
        return ret;
    }
    ...
    public RatPoly add(RatPoly p) {
        checkRep();
        RatPoly ret = new RatPoly(new ArrayList<>());
        ...
        for(RatTerm r : p.terms) {
            sortedInsert(ret.terms, r);
        }
        checkRep();
        return ret;
    }
    ...
    public RatPoly sub(RatPoly p) {
        ...
        for(RatTerm r : p.terms) {
            sortedInsert(ret.terms, r.negate());
    ...
    public RatPoly mul(RatPoly p) {
        ...
        for(RatTerm q : terms) {
            for(RatTerm r : p.terms) {
                RatNum c = r.getCoeff().mul(q.getCoeff());
                int ex = r.getExpt() + q.getExpt();
                sortedInsert(ret.terms, new RatTerm(c, ex));
            }
        }
        checkRep();
        return ret;
    ...
    public RatPoly div(RatPoly p) {
        ...
        RatPoly dividend = new RatPoly(new ArrayList<>());
        for (RatTerm term : terms) {
            dividend.terms.add(term);
        }

        if (p.isNaN()||isNaN()||p.terms.size() == 0) {
            return NaN;
        }
        if(terms.size()==0) {
            return ZERO;
        ...
        while (dividend.degree() >= p.degree() && dividend.terms.size() > 0) {
            int e = dividend.degree() - p.degree();
            RatNum c = (dividend.getTerm(dividend.degree()).getCoeff()).div(p.getTerm(p.degree()).getCoeff());
            sortedInsert(quotient.terms, new RatTerm(c, e));
            dividend = dividend.sub((new RatPoly(new RatTerm(c, e))).mul(p));
        }
        checkRep();
        return quotient;
    }
