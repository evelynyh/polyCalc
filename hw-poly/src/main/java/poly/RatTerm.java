...
public final class RatTerm {
    ...
    public boolean isZero() {
        return (this.coeff.equals(ZERO.coeff));
    }
    ...
    public double eval(double d) {
        ...
        double result = Math.pow(d, expt);
        return result * coeff.doubleValue();
    }
    ...
    public RatTerm negate() {
        ...
        return new RatTerm(coeff.negate(), this.expt);
    }
    ...
    public RatTerm add(RatTerm arg) {
        if ((this.expt != arg.expt)&&!(isZero()||arg.isZero()||arg.isNaN()||isNaN())) {
            throw new IllegalArgumentException();
        }
        if (arg.isNaN()||isNaN()) {
            return NaN;
        }
        RatNum c = arg.coeff.add(this.coeff);
        return new RatTerm(c, Math.max(expt, arg.expt));
    }
    ...
    public RatTerm sub(RatTerm arg) {
        ...
        RatNum c = this.coeff.sub(arg.coeff);
        return new RatTerm(c, Math.max(expt, arg.expt));
    }
    ...
    public RatTerm mul(RatTerm arg) {
        ...
        RatNum c = this.coeff.mul(arg.coeff);
        int e = expt + arg.getExpt();
    ...
    public RatTerm div(RatTerm arg) {
        ...
        RatNum c = this.coeff.div(arg.coeff);
        int e = expt - arg.getExpt();
        return new RatTerm(c, e);
    }
